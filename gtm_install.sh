...
COIN_TGZ='https://raw.githubusercontent.com/batagope/gtm-mn-auto/master/gentariumcore-v1.1.0-linux.tar.gz'
...
purgeOldInstallation() {
    ...
    #kill wallet daemon
    sudo killall gentariumd > /dev/null 2>&1
    #remove old ufw port allow
    sudo ufw delete allow 17017/tcp > /dev/null 2>&1
    #remove old files
    if [ -d "~/.gentarium" ]; then
        sudo rm -rf ~/.gentarium > /dev/null 2>&1
    fi
    #remove binaries and MCT utilities
    cd /usr/local/bin && sudo rm gentarium-cli gentarium-tx gentariumd > /dev/null 2>&1 && cd
...
function important_information() {
 ...
 echo -e "${GREEN}mct-cli masternode status${NC}"
 echo -e "${GREEN}mct-cli getinfo.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
